alter table Configuracao alter column ControlaFidelidade nvarchar(max);
go
...
@ExigeVendedorSN bit,
@GCM nvarchar(max),
@ImpressoraCozinha nvarchar(max),
@ImpressoraEntrega nvarchar(max),
@ImpressoraCopaBalcao nvarchar(max),
@CobrancaProporcionalSN bit,
@TipoImpressao nvarchar(50)
as 
begin
insert into Configuracao (ImpViaCozinha,UsaDataNascimento,UsaLoginSenha,QtdCaracteresImp,
						  ...
						  CidadesAtendidas,ExigeVendedorSN,GCM,ImpressoraCozinha,ImpressoraEntrega,ImpressoraCopaBalcao,
						  CobrancaProporcionalSN,TipoImpressao)
						  values
                            (@ImpViaCozinha,@UsaDataNascimento,@UsaLoginSenha,@QtdCaracteresImp,
							@ControlaEntregador,@ProdutoPorCodigo,@Usa2Telefones,@UsaControleMesa,
							@ImprimeViaEntrega,@ControlaFidelidade,@PedidosParaFidelidade,@DescontoDiaSemana,
							@PrevisaoEntrega,@PrevisaoEntregaSN,@CobraTaxaGarcon,@TamanhoFont,@ImpLPT,@PortaLPT,@EnviaSMS,
							@ViasEntrega,@ViasCozinha,@ViasBalcao,@RepeteUltimoPedido,@RegistraCancelamentos,@DadosApp,
							@Pushauthorization,@Pushapp_id,@CidadesAtendidas,@ExigeVendedorSN,@GCM,@ImpressoraCozinha,
							@ImpressoraEntrega,@ImpressoraCopaBalcao,@CobrancaProporcionalSN,@TipoImpressao)
	end
...
ImpressoraEntrega =@ImpressoraEntrega ,
ImpressoraCopaBalcao = @ImpressoraCopaBalcao,
...
isnull(ImprimiSN,0) as ImprimiSN 
 ...
 ,isnull(Pushapp_id,0) as Pushapp_id
  ...
  ,isnull(GCM,0) as GCM
  ,isnull(ImpressoraCozinha,'') as ImpressoraCozinha
,isnull(ImpressoraEntrega,'') as ImpressoraEntrega
,isnull(ImpressoraCopaBalcao,'') as ImpressoraCopaBalcao
,isnull(CobrancaProporcionalSN,0) as CobrancaProporcionalSN
,isnull(TipoImpressao,'Por Cozinha/Grupo') as TipoImpressao
 from Configuracao
